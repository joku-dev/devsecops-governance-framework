...
The first additional adapter prepares **diagnostic DevSecOps control candidates**
from the existing control-evaluation report. It does not submit observations to
...
records classification and boundaries.
...
Their separate adapter must retain that granularity; marker IDs and marker-level
PASS/failure cannot be invented from those messages. Architecture exceptions
