# Governance Change Request: Operations Process Documentation Migration

## Summary

- Move governance operation process documents from the `docs/operations/` root
  into `docs/operations/processes/`.
- Add an operations processes index page and group process documents in MkDocs
  navigation.
- Update active documentation references to the new process paths.
...
| Reviewed non-source path | `docs/operations/processes/` |
| Register updated? | no |
...
The `docs/operations/` root still contained governance operation process
documents after the guides migration.

Moving these documents into `docs/operations/processes/` separates process
models, enforcement options, waivers, timing and intake verification from
practical guides, evidence handling, platform adapters, agent operations and
status reporting.
...
- The change does not alter source-document intake, policies, controls,
  ...
  release content.
- Moving these documents does not promote them into source documents and does
  not change report-only, warning or blocking behavior.
...
- Review whether the moved process paths are correctly reflected in MkDocs
  navigation, AGENTS, official entrypoints, AI index and active cross-document
  links.
- Historical governance change request and reference-run references were not
  rewritten because they record the path context at the time of the original
  decision.
