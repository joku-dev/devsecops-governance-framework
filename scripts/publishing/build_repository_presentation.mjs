/** Build the repository walkthrough from its editorial JSON source.
...
for(const [i,item] of content.slides.entries()){
 ...
 if(item.layout==='cover'){
  text(slide,item.title,82,182,1120,150,62,foreground,true);
  text(slide,item.lead,86,370,1080,120,34,'#DCE9EB');
  ...
  if(item.layout==='table'){
   ...
   const table=slide.tables.add({rows:item.table.length,columns:2,left:78,top:246,width:1124,height:370,columnWidths:[410,714],values:item.table});
   ...
   for(let r=0;r<item.table.length;r++){
    table.rows[r].height=r===0?56:(item.table.length>5?64:88);
    for(let c=0;c<2;c++){
   ...
   for(const [k,[label,body]] of item.items.entries()){
    text(slide,label,80+k*594,274,528,72,36,C.teal,true);
    text(slide,body,80+k*594,370,524,240,30,C.ink);
   }
  }else if(item.layout==='steps'){
   for(const [k,[label,body]] of item.items.entries()){
    text(slide,label,80,250+k*94,288,76,27,C.teal,true);
    text(slide,body,396,250+k*94,794,84,27,C.ink);
   }
